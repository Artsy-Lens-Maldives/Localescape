...
function bigMap(_latitude, _longitude, element, useAjax) {
    if (document.getElementById(element) != null) {
        var placeMarkers = function placeMarkers(locations) {

            var newMarkers = [];
            for (i = 0; i < locations.length; i++) {
                var marker;
                var markerContent = document.createElement('div');
                markerContent.innerHTML = '<div class="map-marker">' + '<div class="icon">' + '<img src="/assets/img/marker.png">' + '</div>' + '</div>';
        ...
        var ajaxLoadInfobox = function ajaxLoadInfobox(i, marker, newMarkers, locations, _this, id) {
            $.ajax({
        ...
        var openInfobox = function openInfobox(i, marker, newMarkers, locations, _this, htmlContent) {
            if (useAjax == true) {
                console.log(useAjax);
                boxText = htmlContent;
            } else {
                ...
                '</a>';
                */
                '<a href="' + locations[i]["url"] + '" class="infobox-inner">' + '<div class="image-wrapper">' + '<div class="label-wrapper">' + '<figure class="label label-info">' + locations[i]["type"] + '</figure>' + '</div>' + '<div class="wrapper">' + '<div class="info">' + '<h3>' + locations[i]["title"] + '</h3>' + '<figure class="location">' + locations[i]["location"] + '</figure>' + '</div>' + '</div>' + '<div class="image" style="background-image: url(' + locations[i]["image"] + ')"></div>' + '</div>' + '<div class="meta">' + '<span><i class="fa fa-star"></i>' + locations[i]["rating"] + '</span>' + '<span><i class="fa fa-bed"></i>' + locations[i]["beds"] + '</span>' + '</div>' + '</a>';
            }
            infoboxOptions = {
                content: boxText,
